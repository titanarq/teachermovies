...
```bash
gh api -X PUT repos/titanarq/teachermovies/pulls/N/merge -f merge_method=squash -f sha=<verified head sha>
gh api -X DELETE repos/titanarq/teachermovies/git/refs/heads/<head branch>
```
...
- `systemctl --user status teachermovies-guard.timer` — a stopped timer means nothing else below is
  moving.
- `journalctl --user -u teachermovies-guard.service --since "12 hours ago" -o cat | grep -vE "never started|^0 event|nothing to wake"`.
...
took: `journalctl --user -u teachermovies-guard.service --since "10 min ago" -o cat | grep -i nudg` and a
